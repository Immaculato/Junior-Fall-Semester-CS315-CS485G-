...
static unsigned long desiredSolutions = 0;
static unsigned long long backtracks = 0;
...
void solveLargeSudoku(vector<char> sudokuBoard, int index) {
	if (largeSudokuSolved) return;
		...
		if (itr == --(candidates.end())) backtracks+=1;
        }
}
...
int main()  {
	...
	cin.ignore();
	if (userChoice==2) {
		cout << "Please enter the number of solutions you would like to print." << endl;
		cin >> desiredSolutions;
		cin.ignore();
		cout << "Please enter your 9x9 puzzle:" << endl; 
		vector<char> sudokuBoard;
		 ...
		 cout << "Backtracked " << backtracks << " times to find " << solutions << " solution(s)." << endl;
		 if (desiredSolutions != solutions) {
			cout << "Could only find " << solutions << " solution(s) out of the the desired "<<desiredSolutions <<endl;
		 }  
