...
void sendError(int response, int connectfd){
	...
	if( response != 0 ){
		Rio_writen(connectfd, &error, 4); 
	}

	else
		Rio_writen(connectfd, &zero , 4);
...
int mcput(char * filename, int connectfd, int datalen, rio_t * rio){
	...
	char *file;
	//we send in chunks of 10,000 bytes, so read athose appropriately.
...
int mcget(char * filename, int connectfd) {
	...
	unsigned int networkOrderSize = htonl(allFiles[i].fileSize);
        ...
        rio_writen(connectfd, &networkOrderSize, 4);
...
void application(int connectfd, int secretKey ){
	...
	Rio_readn(connectfd, &rawNetBytes, 4);
	keyUser = ntohl(rawNetBytes);
	printf("Secret Key = %u\n", keyUser);
	
	valid = validate(secretKey, keyUser);

	Rio_readn(connectfd, &rawTypeReq, 4);
	typeRequest = ntohl(rawTypeReq);
	char typeString[5];
	switch(typeRequest) {
		case 0: strncpy(typeString, "get", 4);
			break;
		case 1: strncpy(typeString, "put", 4);
			break;
		case 2: strncpy(typeString, "del", 4);
			break;
		case 3: strncpy(typeString, "list", 5);
			break;
		default: strncpy(typeString, "???", 4);
			 valid = 0;
			 break;
	}
	...
	if(valid){
		switch(typeRequest){
			case 0:
				...
				break;
			case 1:
				Rio_readn(connectfd, filename, 80);
				Rio_readn(connectfd, &inputLength, 4);
